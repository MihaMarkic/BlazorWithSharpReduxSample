...
namespace Sharp.Redux
{
        ...
        where TReducer: IReduxReducer<TState>
    {
        ...
        //    await StopAsync();
        //    state = newState;
        //    await notificationFactory.StartNew(st => OnStateChanged(new StateChangedEventArgs<TState>(new StateResetAction(), (TState)st)), state).Unwrap();
        //}
        ///// <summary>
        ...
        {
            //queue.Add(action);
            OnStateChanged(new StateChangedEventArgs<TState>(new InitAction(), state));
        ...
        internal void ProcessAction(ReduxAction action)
        {
            var oldState = state;
            state = reducer.Reduce(state, action);
            if (!ReferenceEquals(oldState, state))
            {
                OnStateChanged(new StateChangedEventArgs<TState>(action, state));
            }
        }
